...
### 2026-02-13 📋 請求：用戶身份識別機制改善（帳號讀取失敗 → 全部資料斷鏈）
...
目前系統用戶身份完全綁定 Google/Apple 社交帳號，沒有獨立的 userId 機制。當社交帳號讀取失敗時（Token 過期、網路異常、第三方服務暫時不可用），整條資料鏈直接斷裂：

```
帳號讀取失敗 → 沒有 userId → 無法呼叫任何 API → 圖鑑、行程、成就全部拿不到
```
...
| 場景 | 現況 |
|------|------|
| Google/Apple Token 過期 | 用戶被強制登出，所有資料消失直到重新登入 |
| 第三方登入服務暫時故障 | 用戶完全無法使用 App |
| 登出 → 快速重新登入 | 可能出現競態條件（#009 教訓） |
| 想做訪客模式 | 無法實作，因為沒有社交帳號就沒有身份 |

**建議方案**

**方案 A（推薦）：後端產生獨立 userId**

```
用戶首次登入（Google/Apple）
  → 後端產生獨立 userId（UUID 或其他格式）
  → 社交帳號綁定到這個 userId
  → 之後所有 API 都用這個 userId（透過 JWT Token）
  → 社交帳號只負責「驗證身份」，不負責「代表身份」
```

好處：
- 社交帳號暫時讀不到時，只要 JWT Token 還有效就能繼續用
- 未來要做訪客模式、帳號合併、換綁社交帳號都容易
- 業界標準做法（Firebase、Supabase 都是這樣）

**方案 B：延長 JWT Token 有效期 + 本地快取身份**

```
登入成功 → JWT Token 存本地（已做）
  → Token 有效期間不需要重新驗證社交帳號
  → 搭配 Token Refresh 機制（已在 sync-backend.md 請求過）
```

好處：
- 改動最小，不用動資料庫架構
- 但根本問題沒解決（Token 真的過期還是斷鏈）

**方案 C：方案 A + 訪客模式支援**

```
在方案 A 基礎上，允許不綁社交帳號的匿名 userId
  → 訪客按「先逛逛」→ 後端產生匿名 userId
  → 可以瀏覽、體驗基本功能
  → 要做進階操作（扭蛋、行程）時提示綁定社交帳號
```
...
- 目前已有 401 攔截器（Token 過期自動登出）
- 已請求 Token Refresh API（見下方歷史回報）
- 如果後端採用方案 A，前端改動很小 — JWT Token 裡帶獨立 userId 即可，現有 API 呼叫不受影響

**請後端評估哪個方案可行，以及預估改動範圍。**
...
| # | 日期 | 主題 | 狀態 |
|---|------|------|------|
| 請求 | 02-13 | 用戶身份識別機制改善（社交帳號斷鏈 → 全部資料消失） | 🟡 |
